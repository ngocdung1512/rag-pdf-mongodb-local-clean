...
st.set_page_config(page_title="PDF Documentation", page_icon="📝")
...
def ocr_on_pdf(pdf_path):
    try:
        ...
        if not tesseract_path:
            raise EnvironmentError("TESSERACT_PATH not set in environment variables.")
        if not poppler_path:
            raise EnvironmentError("POPPLER_PATH not set in environment variables.")
        ...
        if not images:
            logging.warning(f"No images were generated from PDF: {pdf_path}")
            return ""
        ...
        logging.info(f"OCR extracted {len(extracted_text)} characters from {pdf_path}")
...
def process_pdf(pdf):
    try:
        ...
        file_size = path.getsize(temp_pdf_path) / (1024 * 1024)  # Size in MB
        ...
        if not text_from_pdf:
            ...
            ocr_text = ocr_on_pdf(temp_pdf_path)
            logging.info(f"OCR extracted {len(ocr_text)} characters from {pdf.name}")
            return ocr_text
...
def get_vectorstore(text_chunks: List[str], source_name: str = "unknown.pdf") -> MongoDBAtlasVectorSearch:
    ...
    ids = [vector_search.add_texts([chunk], [metadata])[0]
           for chunk, metadata in zip(text_chunks, metadatas)]

    logging.info(f"Added {len(ids)} embeddings to the vector store with metadata source={source_name}")
...
def main():
    ...
    with st.sidebar:
        st.subheader("Explore Your PDF Documents")
        ...
        if st.button("Process") and uploaded_files:
            with st.spinner("Processing..."):
                for pdf in uploaded_files:
                    ...
                    vectorstore = get_vectorstore(text_chunks, source_name=pdf.name)
    ...
    # Main page content
    st.title(":rainbow[Smart Chat with Documents]  :open_book:")
